...
このモードでは、FSx for Lustreファイルシステムが作成され、指定されたS3バケットからデータを自動的にインポートします。ただし、S3へのエクスポートは自動的には行われず、ジョブ完了後に明示的なデータリポジトリタスクを実行してデータをS3にエクスポートします。
...
10. **データリポジトリタスク作成** (CreateDataRepositoryTask)
