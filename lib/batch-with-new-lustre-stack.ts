...
export class BatchWithNewLustreStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const ecrRepository = new ecr.Repository(this, 'BatchJobRepository', {
      repositoryName: 'batch-with-new-lustre-job',
      removalPolicy: cdk.RemovalPolicy.DESTROY, // 開発環境用
    ...
    // FSx for Lustreファイルシステムの作成
    const lustreFileSystem = new fsx.LustreFileSystem(this, 'LustreFileSystem', {
      vpc,
      vpcSubnet: vpc.privateSubnets[0],
      securityGroup: lustreSecurityGroup,
    ...
    // データリポジトリの関連付け
    new fsx.CfnDataRepositoryAssociation(this, 'LustreDataRepositoryAssociation', {
      fileSystemId: lustreFileSystem.fileSystemId,
    ...
    const stepFunctionsRole = new iam.Role(this, 'BatchWithNewLustreStateMachineRole', {
      assumedBy: new iam.ServicePrincipal('states.amazonaws.com'),
      inlinePolicies: {
        'BatchAndLustrePermissions': new iam.PolicyDocument({
          statements: [
            // EC2起動テンプレートの権限
            new iam.PolicyStatement({
              effect: iam.Effect.ALLOW,
    ...
    const createJobDefinition = new tasks.CallAwsService(this, 'CreateJobDefinition', {
      ...
      parameters: {
        JobDefinitionName: sfn.JsonPath.format('lustre-job-definition-{}', lustreFileSystem.fileSystemId),
        Type: 'container',
        ContainerProperties: {
    ...
    const submitJob = new tasks.CallAwsService(this, 'SubmitJob', {
      ...
      action: 'submitJob',
      parameters: {
        JobName: sfn.JsonPath.format('lustre-job-{}', lustreFileSystem.fileSystemId),
        JobQueue: jobQueue.ref,
        JobDefinition: sfn.JsonPath.format('lustre-job-definition-{}', lustreFileSystem.fileSystemId)
      },
      iamResources: ['*'],
    ...
    const definition = createLaunchTemplate
    ...
    // メトリクスチェックのフローを追加
    checkMetricsTask.next(waitForNextCheck);
    waitForNextCheck.next(checkMetricsTask);

    new sfn.StateMachine(this, 'BatchWithNewLustreStateMachine', {
      definition,
